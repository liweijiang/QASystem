...
class Attention(object):
    ...
    def calculate(self, h, u, h_mask, u_mask, JX, JQ, dropout = 1.0):
        ...
        return tf.concat([h, u_a, h_0_u_a, h_0_h_a], 2)
...
class Encoder(object):
    ...
    def encode(self, inputs, mask, encoder_state_input, dropout = 1.0):
        ...
        lstm_fw_cell = tf.contrib.rnn.LSTMCell(self.state_size, state_is_tuple=True, reuse=tf.get_variable_scope().reuse)
...
class Decoder(object):
    ...
    def decode(self, g, context_mask, JX, dropout = 1.0):
        ...
        e_input = tf.concat([m_2, m_2 * s_prob, s_prob], 2)
    ...
    def get_logit_start_end(self, X, JX):
        ...
        pred2_1 = tf.concat([pred1, pred2_0], 1)
...
class QASystem(object):
    ...
    def export_model(self, session, export_path_base, dataset=None, vocab=None):
        ...
        builder.save()
        logging.info("Finishing exporting trained model to {}".format(export_path))

    def evaluation(self, session, dataset, train_dir, vocab):
        """ evaluate the model """
        validation_set = dataset['validation']
        logging.info("-- validation --")
        self.validate(session, validation_set)
